...
const TaxView: React.FC<Props> = ({ data, onUpdate }) => {
      ...
      Object.entries(importedDays).forEach(([date, importedEntry]) => {
          if (!newData.trades[date]) {
              ...
              importedEntry.trades.forEach(newTrade => {
                  // If it's an aggregated trade (e.g. from Exec Report), check if we already have one for this symbol
                  // If so, replace it (assuming the import is the source of truth), otherwise add it.
                  const existingIdx = existingDay.trades.findIndex(existing => 
                      existing.inst === newTrade.inst && 
                      (existing.strategy?.includes('Agg') && newTrade.strategy?.includes('Agg'))
                  );

                  if (existingIdx > -1 && newTrade.strategy?.includes('Agg')) {
                      // Update existing aggregation
                      existingDay.trades[existingIdx] = newTrade;
                  } else {
                      // Standard duplicate check for manual trades
