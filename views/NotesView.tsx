    ...
    setTimeout(async () => {
        try {
            ...
            if (result.movedCount > 0) {
                ...
                let expenseMsg = "";
                const newDailyExpenses = { ...data.dailyExpenses };
                if (result.newDailyExpenses.length > 0) {
                    result.newDailyExpenses.forEach(exp => {
                        const year = exp.date.split('-')[0];
                        if (!newDailyExpenses[year]) newDailyExpenses[year] = [];
                        const exists = newDailyExpenses[year].some(e => e.receiptId === exp.receiptId);
                        if (!exists) newDailyExpenses[year].push(exp);
                    });
                    expenseMsg = `, ${result.newDailyExpenses.length} Ausgaben erfasst!`;
                }

                onUpdate({ ...data, notes: newNotes, tax: { ...data.tax, expenses: newExpenses }, dailyExpenses: newDailyExpenses });
                
                const msg = `${result.movedCount} Dateien importiert${taxMsg}${expenseMsg}`;
  ...
  return (
    ...
    >
      ...
      <div className={`flex-1 flex flex-col bg-gray-50/30 ${selectedNoteId ? 'fixed inset-0 z-[100] bg-white md:static h-[100dvh]' : 'hidden md:flex'}`}>
         {selectedNote ? (
             <>
                <div className="px-4 pb-3 pt-[calc(1rem+env(safe-area-inset-top))] border-b border-gray-100 bg-white flex flex-wrap items-center justify-between shrink-0 gap-y-2">
                    <div className="flex items-center gap-3 flex-1 mr-2 overflow-hidden min-w-[200px]">
                        {/* Mobile Back Button */}
                        <button onClick={() => setSelectedNoteId(null)} className="md:hidden p-3 -ml-3 text-gray-500 hover:bg-gray-100 rounded-full shrink-0">
                           <ArrowLeft size={20} />
                        </button>
